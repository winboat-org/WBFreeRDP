...
The final local regression run passed **24 build/test groups** against the
19-patch source tree. A separate verification reapplied the series to pristine
...
## Latest Windows integration check

Patches 0018–0019 passed six native fading-tooltip cycles on the final release:
...
checks**, and **4 switches to a native Linux window** on the final uninstrumented
...
Those matrices were not repeated for patches 0016–0019. Their original release hash and
...
The current `validation-summary.json` records the 19-patch release hash, local
checks and tooltip integration results. The preceding summaries are preserved
in `validation-summary-17patch.json` and `validation-summary-16patch.json`.
